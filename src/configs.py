...
@dataclass
class RankDataArguments:
    """
    Arguments pertaining to ranking preference data.
    """
    ...
    human_prefix: Optional[str] = field(
        default="Human:", metadata={"help": "human_prefix template"}
    )
    human_suffix: Optional[str] = field(
        default="", metadata={"help": "human_suffix template"}
    )    
    assistant_prefix: Optional[str] = field(
        default="Assistant:", metadata={"help": "assistant_prefix template"}
    )
    assistant_suffix: Optional[str] = field(
        default="", metadata={"help": "assistant_suffix template"}
    )        
...
@dataclass
class DRPOConfig(transformers.TrainingArguments):
    """
    Arguments related to the DRPO training process itself. For all parameters, see: https://huggingface.co/docs/transformers/v4.26.1/en/main_classes/trainer#transformers.TrainingArguments
    """
    ...
    loss_type: Optional[str] = field(default="diffndcg", metadata={"help": ("The loss type for DRPO.")})
    loss_config: Optional[Dict[str, Any]] = field(default=None, metadata={"help": ("The loss type for DRPO.")})
    use_ref: bool = field(default=True)
    mean_logps: bool = field(default=False)
    score_type: Optional[str] = field(default="dpo", metadata={"help": ("The loss type for computing score.")})
    score_config: Optional[str] = field(default=None, metadata={"help": ("The score config for computing score.")})
    
    kl_coef : float = field(default=None)
