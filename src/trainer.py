...
class DRPODataCollatorWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # print(padded_batch['rank_target_input_ids'])
        # print(padded_batch['rank_target_labels'])
...
class DRPOTrainer(Trainer):
    ...
    def score_scale(self,scores,eps=0.00000001):
    #     s_max ,_ =  torch.max(scores,dim=-1,keepdim=True)
    #     s_min ,_ =  torch.min(scores,dim=-1,keepdim=True)
    #     # print('score!!!!!!',scores,s_max,s_min)
    #     scores = (scores - s_min + eps ) / (s_max - s_min + eps)
        return scores   
    ...
    def drpo_loss(
        ...
        """
        # if 'odds' in self.psi_type:
        #     odds = policy_logps - torch.log(1 - torch.exp(policy_logps))
        #     pred_scores = odds
        #     print('odds pred_scores',pred_scores)
        # elif 'ad' in self.psi_type:
        #     pred_scores = policy_logps
        #     # print('policy logps',policy_logps)
        #     print('ad pred_scores',pred_scores)
    ...
    def cross_entropy_loss(self,logits, labels):
        ...
        # Flatten the tokens
        print('ce shape',logits.shape,labels.shape)
    ...
    def concatenated_forward(
        ...
        """
        # print("input" ,batch['rank_target_input_ids'].shape)
        # print("mask" ,batch['rank_target_attention_mask'].shape)
        # print("labels" ,batch['rank_target_labels'].shape)
        ...
        # print(
        #     rank_target_input_ids.device,
        #     rank_target_attention_mask.device,
        #     rank_target_labels.device,
        # )
        outputs = model(rank_target_input_ids,
                           attention_mask=rank_target_attention_mask
                           )
        # print('out->',all_logits)
        # all_logps = get_rank_batch_logps(all_logits, rank_target_combined_labels)
        ...
        # all_logits = all_logits.reshape(N,R,L,-1)
    ...
    def get_batch_loss_metrics(
        ...
        # self.loss(policy_logps, reference_logps,batch['target_scores'])
                
        # if self.loss_type == 'pro':
        #     pred_scores = policy_logps
        # elif self.score_type == 'ad':
        #     logps_margin  = (policy_logps - reference_logps)
        #     forward_kl = _tdpo_get_batch_position_forward_kl(all_logits,ref_logits,labels=batch['rank_target_labels'].reshape(N*R, -1))
        #     # print(logps_margin.shape,all_logits.shape,ref_logits.shape)
        #     # print('logps',policy_logps,reference_logps)
        #     # print('forward_kl',forward_kl.shape)
        #     rank_forward_kl = forward_kl.reshape(N,R).detach()
        #     # print('rank_forward_kl',rank_forward_kl.shape)
        #     pred_scores = self.beta * (logps_margin + rank_forward_kl)
        #     # print('scores',pred_scores,logps_margin,rank_forward_kl )
            
        #     max_kl = rank_forward_kl.gather(-1, max_idx).detach().squeeze()
        #     min_kl = rank_forward_kl.gather(-1, min_idx).detach().squeeze()
        #     metrics[f"{prefix}kl/max"] = max_kl.detach().cpu().mean()
        #     metrics[f"{prefix}kl/min"] = min_kl.detach().cpu().mean()

        # elif self.score_type == 'prob':
        #     assert self.mean_logps
        #     pred_scores = policy_logps
        # elif self.score_type == 'prob+rank':
        #     assert self.mean_logps
        #     pred_scores = policy_logps
        #     rank_indices = torch.argsort(-batch['target_scores'], dim=-1)
        #     rank_true = torch.argsort(rank_indices, dim=-1)
        #     pred_scores = pred_scores + rank_true * self.rank_coef

        
        # elif self.score_type == 'prob+kl':
        #     raise NotImplementedError

        # elif self.score_type == 'ema':
            
        #     logps_margin =  (policy_logps - reference_logps)
        #     pred_scores = logps_margin
        #     for r in range(R):
        #         # 获取除当前元素外的 R-1 个元素
        #         avg = torch.cat((logps_margin[:, :r], logps_margin[:, r+1:]), dim=1)
        #         # print(avg.shape)
        #         avg = torch.mean(avg,dim=1).detach()
        #         # print(avg.shape)
        #         # print(r,pred_scores[:,r],avg)
        #         pred_scores[:,r] -= avg
            
        #     pred_scores = self.beta * pred_scores
        #     # print('scores',pred_scores,self.beta * (policy_logps - reference_logps))
        # elif self.score_type == 'dpo':
        ...
        if self.add_klloss:
            # forward_kl = _tdpo_get_batch_position_forward_kl(all_logits,ref_logits,labels=batch['rank_target_labels'].reshape(N*R, -1),average_log_prob=self.mean_logps)  
            # rank_forward_kl = forward_kl.reshape(N,R).detach()
            # self.kl_coef
            # pred_scores = self.beta * mean_logps + self.kl_coef * rank_forward_kl
            # print(rank_forward_kl.shape,rank_forward_kl.gather(-1, min_idx).detach().shape)
            forward_kl = _tdpo_get_batch_position_forward_kl(all_logits,ref_logits,labels=batch['rank_target_labels'].reshape(N*R, -1),average_log_prob=self.mean_logps)  
            rank_forward_kl = forward_kl.reshape(N,R)
            max_kl = rank_forward_kl.gather(-1, max_idx).detach().squeeze()
            min_kl = rank_forward_kl.gather(-1, min_idx).detach().squeeze()
            metrics[f"{prefix}kl/max"] = max_kl.detach().cpu().mean()
            metrics[f"{prefix}kl/min"] = min_kl.detach().cpu().mean()
            
            kl_loss =  self.kl_coef * ( torch.abs( rank_forward_kl - rank_forward_kl.gather(-1, min_idx).detach() )   ).mean()
            metrics[f"{prefix}_kl_loss"] = kl_loss.detach().cpu().mean()
            losses += kl_loss
            print(f'pro loss -> drpo: {drpo_losses_item},kl_coef:{self.kl_coef},L:{R}, kl loss:{kl_loss},all:{losses}')
